...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR("  ❌ Invalid AWS credentials"))
            return
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"  ❌ Error creating S3 client: {e}"))
            return
        ...
        try:
            ...
            response = s3_client.list_objects_v2(
                Bucket=bucket_name, MaxKeys=1  # Just check if we can list
            )
            ...
            if error_code == "NoSuchBucket":
                ...
                self.stderr.write(self.style.ERROR(f"  ❌ Error accessing bucket: {e}"))
            return
        ...
        if test_upload:
            ...
            try:
                ...
                self.stdout.write(
                    f'  ✅ File verified: {response["ContentLength"]} bytes'
                )
