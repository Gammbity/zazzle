...
class OrderDetailSerializer(serializers.ModelSerializer):
    ...
    shipping_method_info = ShippingMethodSerializer(
        source="shipping_method", read_only=True
    )
...
class CheckoutSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if delivery_method == Order.DeliveryMethod.PICKUP:
            ...
            raise serializers.ValidationError(
                {"shipping_address": "Manzil kiritilishi shart."}
            )
    ...
    def _select_nearest_center(delivery_method, attrs):
        ...
        field = (
            "supports_pickup"
            if delivery_method == Order.DeliveryMethod.PICKUP
            else "supports_delivery"
        )
        candidates = list(
            ProductionCenter.objects.filter(is_active=True, **{field: True})
        )
        if not candidates:
            return None
...
class PaymentInitSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if order.status not in ["NEW", "PAYMENT_PENDING"]:
            raise serializers.ValidationError(
                {"order_id": "Order is not eligible for payment."}
            )
