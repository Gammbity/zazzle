...
# ---------------------------------------------------------------------------
# Social login (Google / Facebook / Telegram) — custom cookie-based OAuth,
# not allauth's own social flow. Empty by default; each provider's login
# endpoint 503s until its credentials are configured. Client IDs/bot username
# are not secret and are exposed via GET /api/auth/oauth/config/ so the
# frontend can initialize each provider's SDK without duplicating them.
# ---------------------------------------------------------------------------
...
RENDERING_FONT_DEFAULT_FAMILY = config(
    "RENDERING_FONT_DEFAULT_FAMILY", default="sans-serif"
)
RENDERING_FONT_DEFAULT_SIZE = config(
    "RENDERING_FONT_DEFAULT_SIZE", default=16, cast=int
)
