...
class ProductVariantDetailSerializer(ProductVariantSerializer):
    ...
    def get_applicable_templates(self, obj):
        ...
        templates = obj.product_type.asset_templates.filter(is_active=True).order_by(
            "sort_order", "template_type"
        )
    ...
    def get_available_variants(self, obj):
        ...
        for variant in active_variants:
            if variant.color and variant.color_hex:
                colors.append({"name": variant.color, "hex": variant.color_hex})
        ...
        return {"sizes": sizes, "colors": unique_colors}
