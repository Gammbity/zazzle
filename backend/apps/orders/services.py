...
def can_manage_order(order: Order, user) -> bool:
    return user.is_center_staff_of(order.production_center_id) or is_assigned_manager(
        order, user
    )
...
def validate_status_transition(order: Order, new_status: str) -> str | None:
    ...
    if new_status == "IN_PRODUCTION" and order.status not in [
        "READY_FOR_PRODUCTION",
        "IN_PRODUCTION",
        "QUALITY_CHECK",
    ]:
    ...
    if new_status == "QUALITY_CHECK" and order.status not in [
        "IN_PRODUCTION",
        "QUALITY_CHECK",
    ]:
    ...
    if new_status == "COMPLETED" and order.status not in (
        "READY_FOR_PICKUP",
        "READY_FOR_DELIVERY",
        "COMPLETED",
    ):
...
def build_order_stats(
    ...
    *,
    ...
    aggregated = queryset.aggregate(
        ...
        in_production_orders=Count(
            "id", filter=Q(status__in=PRODUCTION_ACTIVE_STATUSES)
        ),
    ...
    orders_by_delivery_method = [
        {
            ...
            "label": str(
                delivery_labels.get(row["delivery_method"], row["delivery_method"])
            ),
            "count": row["count"],
        }
        for row in (
