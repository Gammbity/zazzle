...
from .state import transition as order_transition
...
def order_stats(request):
    ...
    if (
        ...
        or user.is_production_admin
        or user.is_production_manager
    ):
...
def payment_init(request):
    ...
    with transaction.atomic():
        (
            transaction_obj,
            created,
        ) = PaymentTransaction.objects.select_for_update().get_or_create(
...
def assign_order(request, order_id: int):
    ...
    return Response(
        OrderAssignmentSerializer(assignment).data, status=status.HTTP_200_OK
    )
