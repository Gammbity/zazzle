    ...
    def handle(self, *args, **options):
        ...
        for template in templates:
            try:
                ...
                self.stdout.write(
                    f'  ✅ Saved: {template["name"]} ({file_size:,} bytes)'
                )
                self.stdout.write(
                    f'     Design area: {area["width"]}x{area["height"]} at ({area["x"]}, {area["y"]})'
                )
        ...
        if generated_count > 0:
            self.stdout.write("\nNext steps:")
            self.stdout.write(
                "1. Upload templates to S3: python manage.py upload_mockup_templates"
            )
            self.stdout.write(
                "2. Create sample drafts: python manage.py create_sample_drafts"
            )
