...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for family in families_to_test:
            ...
            if family not in font_config:
                self.stderr.write(
                    f"  Font family '{family}' not found in configuration"
                )
                error_count += 1
                continue
    ...
    def _test_font_family(
        self, family_name, family_config, font_size, text, output_dir
    ):
        """Test rendering a specific font family."""

        weights = (
            ["regular", "bold", "italic"]
            if "bold" in family_config or "italic" in family_config
            else ["regular"]
        )
        ...
        for weight in weights:
            try:
                ...
                if not os.path.exists(font_path):
                    ...
                    for fallback in fallback_fonts:
                        try:
                            ...
                            fallback_path = os.path.join(
                                os.path.dirname(font_path), fallback
                            )
                    ...
                    if not font_loaded:
                        self.stdout.write(
                            f"    {weight}: Font file not found: {font_path}"
                        )
                        continue
                ...
                try:
                    ...
                    self.stdout.write(
                        f"    {weight}: Using default font (original font failed to load)"
                    )
                ...
                draw.text(
                    (x, y + 80), info_text, font=info_font, fill=(128, 128, 128, 255)
                )
                ...
                self.stdout.write(
                    f"    {weight}: ✓ Rendered successfully → {output_filename}"
                )
