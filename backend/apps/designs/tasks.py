...
from .models import Draft, DraftAsset, MockupRender, ProductMockupTemplate
...
class S3Manager:
    ...
    def upload_image(
        self, image: Image.Image, s3_key: str, format: str = "JPEG", quality: int = 90
    ) -> dict[str, int]:
...
class FontManager:
    ...
    def get_font(
        self, family: str, size: int, weight: str = "normal"
...
class TextRenderer:
    ...
    def render_text_layer(self, image: Image.Image, text_layer: dict) -> Image.Image:
        ...
        try:
            ...
            font_size = max(
                settings.RENDERING_FONT_SIZE_MIN,
...
class MockupCompositor:
    ...
    def render_mockup(self, config: RenderConfig) -> tuple[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Rendering mockup for draft {draft.uuid} with template {template.name}"
            )
            ...
            result_image = self._composite_design_on_mockup(
                design_image, mockup_image, template
            )
            ...
            metadata = self.s3_manager.upload_image(
                result_image, output_key, format="JPEG", quality=config.jpeg_quality
            )
            ...
            self.s3_manager.upload_image(
                thumbnail, thumbnail_key, format="JPEG", quality=85
            )
    ...
    def _create_design_canvas(
        self, draft: Draft, template: ProductMockupTemplate
    ) -> Image.Image:
    ...
    def _composite_asset(
        self, canvas: Image.Image, asset_image: Image.Image, asset: DraftAsset
    ) -> Image.Image:
        ...
        try:
            ...
            if scale_x != 1.0 or scale_y != 1.0:
                ...
                asset_image = asset_image.resize(
    ...
    def _composite_design_on_mockup(
        self, design: Image.Image, mockup: Image.Image, template: ProductMockupTemplate
    ) -> Image.Image:
        ...
        if template.perspective_matrix:
            design = self._apply_perspective_transform(
                design, template.perspective_matrix
            )
        ...
        if design.mode == "RGBA":
            result.paste(
                design, (template.design_area_x, template.design_area_y), design
            )
        else:
    ...
    def _apply_perspective_transform(
        self, image: Image.Image, matrix: list[float]
    ) -> Image.Image:
...
def render_draft_preview(self, render_id: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            raise MockupRender.DoesNotExist(
                f"Render job not found: {render_id}"
            ) from exc
        ...
        if getattr(settings, "CELERY_TASK_ALWAYS_EAGER", False):
            return {"render_id": render_id, "status": "failed", "error": str(exc)}
...
def cleanup_render_files(older_than_days: int = 30) -> dict:
    ...
    try:
        ...
        old_renders = MockupRender.objects.filter(
            status__in=["completed", "failed"], created_at__lt=cutoff_date
        )
            ...
            try:
                ...
                if render.output_image_s3_key:
                    s3_manager.s3_client.delete_object(
                        Bucket=s3_manager.bucket_name, Key=render.output_image_s3_key
                    )
        ...
        logger.info(
            f"Cleanup completed: {deleted_count} renders deleted, {error_count} errors"
        )
        ...
        return {"error": str(e), "deleted_count": 0, "error_count": 0}
...
def process_design_assets(design_id: int) -> dict:
    ...
    if is_raster:
        try:
            ...
            logger.exception(
                "design.process.metadata_failed", extra={"design_id": design_id}
            )
        ...
        try:
            ...
            # ("/original/..." -> "/optimized/...").
            optimized_name = design.original_file.name.replace(
                "/original/", "/optimized/"
            )
            if optimized_name == design.original_file.name:
                ...
                optimized_name = (
                    f"{base}/optimized/{fname}" if base else f"optimized/{fname}"
                )
            ...
            logger.exception(
                "design.process.optimize_failed", extra={"design_id": design_id}
            )
