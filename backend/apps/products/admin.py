...
from .models import ProductAssetTemplate, ProductType, ProductVariant
...
class ProductTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Specifications",
            {"fields": ("dimensions", "print_area"), "classes": ("collapse",)},
        ),
        ("Status & Ordering", {"fields": ("is_active", "sort_order")}),
        ("Statistics", {"fields": ("variant_count",), "classes": ("collapse",)}),
        (
            "Timestamps",
...
class ProductVariantAdmin(admin.ModelAdmin):
    ...
    search_fields = ["product_type__name", "sku", "size", "color"]
    readonly_fields = [
        "sku",
        "profit_margin",
        "profit_percentage",
    ...
    def set_as_default(self, request, queryset):
        ...
        if queryset.count() == 1:
            ...
            self.message_user(
                request,
                "Please select only one variant to set as default.",
                level="ERROR",
            )
...
class ProductAssetTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Configuration",
            {"fields": ("layer_config", "design_area"), "classes": ("collapse",)},
        ),
        (
            "Applicability",
            {
        ...
        ("Status & Ordering", {"fields": ("is_active", "is_default", "sort_order")}),
        (
            "Timestamps",
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
        ),
