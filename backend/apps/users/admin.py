...
from .models import SocialConnection, User, UserProfile
...
class SocialConnectionInline(admin.TabularInline):
    ...
    fields = ["provider", "provider_username", "is_active", "created_at", "updated_at"]
...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = BaseUserAdmin.fieldsets + (
        ("Role & Permissions", {"fields": ("role", "production_center")}),
        ("Profile Information", {"fields": ("date_of_birth", "avatar", "bio")}),
        (
            "Address",
            {"fields": ("address_line", "city", "state", "postal_code", "country")},
        ),
        ("Business", {"fields": ("is_seller", "store_name", "store_description")}),
    ...
    actions = [
        "make_customer",
        "make_production_manager",
        "make_support",
        "make_production_admin",
        "make_super_admin",
        "deactivate_users",
    ]
    ...
    def make_production_manager(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} users updated to Production Manager role."
        )

    make_production_manager.short_description = (
        "Set selected users as Production Managers"
    )
    ...
    def make_super_admin(self, request, queryset):
        ...
        updated = queryset.update(
            role=User.Role.SUPER_ADMIN, is_staff=True, production_center=None
        )
...
class SocialConnectionAdmin(admin.ModelAdmin):
    ...
    list_display = ["user", "provider", "provider_username", "is_active", "created_at"]
