...
class Command(BaseCommand):
    ...
    def _check_for_issues(self):
        ...
        stuck_renders = MockupRender.objects.filter(
            status="processing", processing_started_at__lt=stuck_threshold
        )
        ...
        high_retry_renders = MockupRender.objects.filter(
            retry_count__gte=3, status="failed"
        )
        ...
        old_pending = MockupRender.objects.filter(
            status="pending", created_at__lt=old_threshold
        )
    ...
    def _retry_failed_renders(self, dry_run, limit):
        ...
        if dry_run:
            ...
            for render in failed_renders:
                self.stdout.write(
                    f"  - {render.render_id} (retries: {render.retry_count})"
                )
            return
        ...
        for render in old_renders:
            try:
                with transaction.atomic():
                    ...
                    if s3_available:
                        for s3_key in [
                            render.output_image_s3_key,
                            render.output_thumbnail_s3_key,
                        ]:
                            if s3_key:
                                try:
    ...
    def _cancel_stuck_renders(self, stuck_hours, dry_run, limit):
        ...
        if dry_run:
            ...
            for render in stuck_renders:
                ...
                self.stdout.write(
                    f"  - {render.render_id} (stuck for {hours:.1f} hours)"
                )
            return
