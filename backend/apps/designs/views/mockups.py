...
def cancel_render(request, render_id):
    ...
    if render_job.task_id:
        try:
            ...
            logger.warning(
                "render.cancel.revoke_failed", extra={"task_id": render_job.task_id}
            )
...
def draft_render_history(request, uuid):
    draft = get_object_or_404(Draft, uuid=uuid, customer=request.user, is_deleted=False)
    renders = MockupRender.objects.filter(draft=draft, user=request.user).order_by(
        "-created_at"
    )
...
def draft_available_templates(request, uuid):
    draft = get_object_or_404(Draft, uuid=uuid, customer=request.user, is_deleted=False)
    templates = (
        ProductMockupTemplate.objects.filter(
            product_type=draft.product_type, is_active=True
        )
        .filter(
            Q(product_variant__isnull=True) | Q(product_variant=draft.product_variant)
        )
