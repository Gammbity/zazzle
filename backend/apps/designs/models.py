...
class DesignCategory(models.Model):
    ...
    icon = models.CharField(
        _("icon class"), max_length=50, blank=True, help_text="CSS icon class"
    )
...
class Design(models.Model):
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="designs"
    )
    design_type = models.CharField(
        _("design type"), max_length=20, choices=DESIGN_TYPES, default="upload"
    )
    status = models.CharField(
        _("status"), max_length=20, choices=DESIGN_STATUS, default="pending"
    )
...
class DesignLicense(models.Model):
    ...
    design = models.OneToOneField(
        Design, on_delete=models.CASCADE, related_name="license"
    )
    license_type = models.CharField(
        _("license type"), max_length=20, choices=LICENSE_TYPES
    )
...
class DesignUsage(models.Model):
    ...
    design = models.ForeignKey(
        Design, on_delete=models.CASCADE, related_name="usage_records"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="design_usage"
    )
...
class DesignCollection(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="design_collections"
    )
...
class Draft(models.Model):
    ...
    editor_state = models.JSONField(
        ...
        blank=True,
        help_text=_(
            "Complete editor state including transforms, positions, layers order"
        ),
    )
        ...
        if self.product_variant and self.product_type:
            if self.product_variant.product_type != self.product_type:
                raise ValidationError(
                    {
                        "product_variant": _(
                            "Product variant must belong to the selected product type."
                        )
                    }
                )
    ...
    def total_file_size(self):
        ...
        return (
            self.assets.filter(is_deleted=False).aggregate(
...
class DraftAsset(models.Model):
    ...
    s3_key = models.CharField(
        _("S3 key"), max_length=500, help_text=_("Unique S3 key for the uploaded file")
    )

    content_type = models.CharField(
        _("content type"), max_length=100, help_text=_("MIME type of the uploaded file")
    )
...
class UploadSession(models.Model):
    ...
    # S3 upload details
    s3_key = models.CharField(
        _("S3 key"), max_length=500, help_text=_("Target S3 key for upload")
    )
    ...
    expected_size = models.PositiveIntegerField(
        _("expected file size"), help_text=_("Expected file size in bytes")
    )

    content_type = models.CharField(
        _("content type"), max_length=100, help_text=_("Expected MIME type")
    )
...
class ProductMockupTemplate(models.Model):
    ...
    product_variant = models.ForeignKey(
        ...
        blank=True,
        help_text=_(
            "Specific variant, if null applies to all variants of the product type"
        ),
    )
...
class MockupRender(models.Model):
    ...
    # Associated draft
    draft = models.ForeignKey(
        Draft, on_delete=models.CASCADE, related_name="mockup_renders"
    )

    # Mockup template used
    mockup_template = models.ForeignKey(
        ProductMockupTemplate, on_delete=models.CASCADE, related_name="renders"
    )

    # User who requested the render
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="mockup_renders"
    )

    # Render status and metadata
    status = models.CharField(
        _("status"), max_length=20, choices=RENDER_STATUS, default="pending"
    )
    ...
    processing_completed_at = models.DateTimeField(
        _("processing completed at"), null=True, blank=True
    )
