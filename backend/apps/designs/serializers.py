...
class DesignDetailSerializer(serializers.ModelSerializer):
    ...
    def get_creator(self, obj):
        ...
        return {
            ...
            "avatar": (
                self.context["request"].build_absolute_uri(obj.created_by.avatar.url)
...
class DraftUpdateSerializer(_DraftJSONValidationMixin, serializers.ModelSerializer):
    ...
    class Meta:
        model = Draft
        fields = ["name", "text_layers", "editor_state", "status"]
    ...
    def validate_status(self, value):
        ...
        if self.instance:
            ...
            allowed_transitions = {
                ...
                Draft.DraftStatus.ARCHIVED: [Draft.DraftStatus.DRAFT],
            }

            if current_status != value and value not in allowed_transitions.get(
                current_status, []
            ):
                raise serializers.ValidationError(
...
class PresignedUploadRequestSerializer(serializers.Serializer):
    ...
    file_size = serializers.IntegerField(
        min_value=1, max_value=50 * 1024 * 1024
    )  # Max 50MB
        ...
        if value:
            ...
            try:
                draft = Draft.objects.get(
                    uuid=value, customer=request.user, is_deleted=False
                )
                return value
    ...
    def validate_session_id(self, value):
        ...
        try:
            session = UploadSession.objects.get(
                session_id=value, user=request.user, is_confirmed=False
            )
            ...
            raise serializers.ValidationError(
                "Upload session not found or already confirmed."
            )
...
class UploadSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["session_id", "created_at", "confirmed_at", "is_expired"]
...
class RenderRequestSerializer(serializers.Serializer):
    ...
    def validate_template_id(self, value):
        ...
        try:
            template = ProductMockupTemplate.objects.get(id=value, is_active=True)
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            if (
                ...
                and template.product_variant != draft.product_variant
            ):
                raise serializers.ValidationError(
...
class RenderResponseSerializer(serializers.Serializer):
    ...
    render_id = serializers.UUIDField(help_text="Unique identifier for this render job")
    status = serializers.CharField(help_text="Current status of the render job")
    message = serializers.CharField(help_text="Human-readable status message")
    estimated_completion = serializers.DateTimeField(
        required=False, help_text="Estimated completion time (if available)"
    )
