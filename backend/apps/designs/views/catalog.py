...
class MyDesignsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Design.objects.filter(created_by=self.request.user).select_related(
            "category"
        )
...
class FeaturedDesignsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Design.objects.filter(is_public=True, status="approved").order_by(
...
class DesignCollectionListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return DesignCollection.objects.filter(user=self.request.user).prefetch_related(
            "designs"
        )
...
def add_to_collection(request, collection_id, design_id):
    collection = get_object_or_404(
        DesignCollection, id=collection_id, user=request.user
    )
    design = get_object_or_404(Design, id=design_id)
    if not (design.is_public or design.created_by == request.user):
        return Response(
            {"error": "Design not accessible"}, status=status.HTTP_403_FORBIDDEN
        )
...
def design_stats(request):
    ...
    stats = {
        ...
        "total_downloads": Design.objects.aggregate(total=Sum("download_count"))[
            "total"
        ]
        or 0,
        ...
        stats.update(
            {
                ...
                "my_collections": DesignCollection.objects.filter(
                    user=request.user
                ).count(),
            }
        )
