...
class ProductionWorkflowPermissionTests(TestCase):
    ...
    def test_status_transitions_enforced(self):
        ...
        resp = self.client.post(status_url, data={'status': 'QUALITY_CHECK'}, format='json')
        self.assertEqual(resp.status_code, 200)
        self.order.refresh_from_db()
        self.assertEqual(self.order.status, 'QUALITY_CHECK')

        # Wrong branch rejected: this order is DELIVERY, not PICKUP.
        resp = self.client.post(status_url, data={'status': 'READY_FOR_PICKUP'}, format='json')
        self.assertEqual(resp.status_code, 400)

        # QUALITY_CHECK -> READY_FOR_DELIVERY (matches delivery_method)
        resp = self.client.post(status_url, data={'status': 'READY_FOR_DELIVERY'}, format='json')
        self.assertEqual(resp.status_code, 200)
        self.order.refresh_from_db()
...
class ProductionCenterCheckoutTests(TestCase):
    ...
    def setUp(self):
        ...
        self.center = _make_center(
            name='Test Showroom', supports_pickup=True, supports_delivery=False,
        )
    ...
    def test_auto_select_picks_nearest_capable_center(self):
        far_center = _make_center(
            name='Far Center', supports_pickup=True, supports_delivery=False,
            latitude=Decimal('40.000000'), longitude=Decimal('65.000000'),
        )
        resp = self.client.post(
            reverse('api-checkout'),
            data=self._checkout_payload(
...
class RoleScopingTests(TestCase):
    ...
    def setUp(self):
        ...
        self.super_admin = User.objects.create_user(
            email='super@example.com', password='password', username='super',
            role=User.Role.SUPER_ADMIN,
        )
        self.admin_a = User.objects.create_user(
            email='admin.a@example.com', password='password', username='admina',
            role=User.Role.PRODUCTION_ADMIN, production_center=self.center_a,
        )
        self.manager_a = User.objects.create_user(
            email='mgr.a@example.com', password='password', username='mgra',
            role=User.Role.PRODUCTION_MANAGER, production_center=self.center_a,
        )
        self.manager_b = User.objects.create_user(
            email='mgr.b@example.com', password='password', username='mgrb',
            role=User.Role.PRODUCTION_MANAGER, production_center=self.center_b,
        )
        customer = User.objects.create_user(
            email='cust@example.com', password='password', username='cust',
        )

        self.order_a = Order.objects.create(
            customer=customer, production_center=self.center_a,
            shipping_name='A', shipping_email='a@example.com',
        )
        self.order_b = Order.objects.create(
            customer=customer, production_center=self.center_b,
            shipping_name='B', shipping_email='b@example.com',
        )
    ...
    def test_production_admin_cannot_manage_other_centers_employees(self):
        self.client.force_authenticate(self.admin_a)
        url = reverse('production:admin-employee-list', kwargs={'center_id': self.center_b.id})
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 403)

    def test_production_admin_can_create_employee_for_own_center(self):
        self.client.force_authenticate(self.admin_a)
        url = reverse('production:admin-employee-list', kwargs={'center_id': self.center_a.id})
        resp = self.client.post(
            url,
            data={
                'username': 'new_mgr', 'email': 'new.mgr@example.com',
                'password': 'Zq9$ProdMgr2026', 'first_name': 'New', 'last_name': 'Manager',
            },
            format='json',
        )
        self.assertEqual(resp.status_code, 201)
