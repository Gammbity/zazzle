    ...
    def get_queryset(self):
        ...
        if sizes:
            queryset = queryset.filter(
                variants__size__in=sizes, variants__is_active=True
            ).distinct()
        ...
        if colors:
            queryset = queryset.filter(
                variants__color__in=colors, variants__is_active=True
            ).distinct()
...
class ProductDetailView(generics.RetrieveAPIView):
    ...
    def get_serializer_class(self):
        ...
        try:
            ...
            return category_serializers.get(
                product_type.category, ProductTypeDetailSerializer
            )
        except:
...
class ProductVariantListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            ProductVariant.objects.filter(product_type_id=product_id, is_active=True)
...
class ProductVariantDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        return ProductVariant.objects.filter(
            product_type_id=product_id, is_active=True
        ).select_related("product_type")
...
def product_filters(request):
    ...
    active_variants = ProductVariant.objects.filter(
        product_type__is_active=True, is_active=True
    )

    # Available sizes across all products
    sizes = sorted(set(variant.size for variant in active_variants if variant.size))

    # Available colors across all products
    colors = []
    color_data = (
        active_variants.exclude(color="")
        .exclude(color_hex="")
    ...
    for item in color_data:
        colors.append({"name": item["color"], "hex": item["color_hex"]})

    # Price range
    price_range = active_variants.aggregate(
        min_price=Min("sale_price"), max_price=Max("sale_price")
    )
...
def product_stats(request):
    ...
    stats = {
        ...
        "categories": ProductType.objects.filter(is_active=True)
        .values("category")
        .annotate(
            count=Count("id"), label=Count("category")  # Will be replaced in the loop
        )
        .order_by("-count"),
    ...
    for category in stats["categories"]:
        category["label"] = category_labels.get(
