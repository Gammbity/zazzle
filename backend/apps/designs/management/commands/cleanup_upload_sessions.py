...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_expired == 0:
            self.stdout.write(self.style.SUCCESS("No expired upload sessions found."))
            return

        self.stdout.write(f"Found {total_expired} expired upload sessions.")
        ...
        while deleted_count < total_expired:
            with transaction.atomic():
                session_ids = list(
                    UploadSession.objects.filter(expires_at__lt=now).values_list(
                        "id", flat=True
                    )[:batch_size]
                )
