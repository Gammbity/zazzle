...
class ProductType(models.Model):
    ...
    category = models.CharField(
        _("category"), max_length=20, choices=ProductCategory.choices, unique=True
    )
...
class ProductVariant(models.Model):
    ...
    product_type = models.ForeignKey(
        ProductType, on_delete=models.CASCADE, related_name="variants"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            ProductVariant.objects.filter(
                product_type=self.product_type, is_default=True
...
class ProductAssetTemplate(models.Model):
    ...
    product_type = models.ForeignKey(
        ProductType, on_delete=models.CASCADE, related_name="asset_templates"
    )
